...
def h_alphabeta_search4(player, board,transposition_table,uses_tt):
            ...
            for y in range(board.column):
                ...
                if board.state[x][y] == player_a:
                    ...
                    for i in range(len(board.directions)):
                        ...
                        LOOKUP_TABLE[(2,0)] = 10
                        LOOKUP_TABLE[(2,1)] = 50
                        LOOKUP_TABLE[(2,2)] = 200
                        LOOKUP_TABLE[(3,0)] = 500
                        LOOKUP_TABLE[(3,1)] = 2000
                        LOOKUP_TABLE[(3,2)] = 20000
                        

                        if (player_a_count,1) in LOOKUP_TABLE:
                            next_x = x + player_a_count*d[0]
                            next_y = y + player_a_count*d[1]
                            next2_x = x + (player_a_count+1)*d[0]
                            next2_y = y + (player_a_count+1)*d[1]
                            prev_x = x - d[0]
                            prev_y = y - d[1]
                            prev2_x = x - 2*d[0]
                            prev2_y = y - 2*d[1]
                            if 0 < next_x < board.row and 0 < prev_x < board.row and 0 < next_y < board.column and 0 < prev_y < board.column :
                               
                                if board.state[next_x][next_y] == 0 and board.state[prev_x][prev_y] == 0:
                                    actions = filter_valid_moves(board.get_valid_moves(player_a))
                                    if (next_x,next_y) in actions and (prev_x,prev_y) in actions :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,2)]

                                    elif (next_x,next_y) in actions or (prev_x,prev_y) in actions :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]
                                    else : 
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,0)]

                                elif board.state[next_x][next_y] == 0 or board.state[prev_x][prev_y] == 0 :
                                    if board.state[next2_x][next2_y] == player_a or board.state[prev2_x][prev2_y] == player_a :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]
                                    else :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]

                            elif 0 < next_x < board.row and 0 < next_y < board.column :

                                if board.state[next_x][next_y] == 0  :
                                    if board.state[next2_x][next2_y] == player_a  :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]
                                    else :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]

                            elif 0 < prev_x < board.row and 0 < prev_y < board.column :

                                if board.state[prev_x][prev_y] == 0  :
                                    if board.state[prev2_x][prev2_y] == player_a  :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]
                                    else :
                                        player_a_possibilities_count += LOOKUP_TABLE[(player_a_count,1)]
